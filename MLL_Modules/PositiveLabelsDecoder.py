...
class PositiveLabelsDecoder(nn.Module):
    ...
    def __init__(self, target_labels_num, target_hidden_size, attention_model='dot', labels_embedding=None,
        ...
        :param rnn_dropout:
        """
        super(PositiveLabelsDecoder, self).__init__()
    ...
    def forward(self, target_sequences, last_state, encoder_outputs):
        ...
        # outputs, hidden = self.RNN_layer(labels_embedded, last_hidden)  # 将词向量和隐藏状态送入RNN中计算

        outputs, state = self.RNN_layer_lstm(labels_embedded, last_state)
        ...
        # TODO 最后算生成值时，要归一化，所以这里还要加上一层softmax
